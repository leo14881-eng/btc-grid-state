...
## v2.17.14 — Proven US runner Bybit geo-block and authorized regional collector

**Confirmed root cause:** GitHub Actions x64 and ARM64 hosted runners both egress in the US (Cloudflare trace `US/IAD`, `US/ORD`), and Bybit's two official global domains return CloudFront 403 with the explicit country-block body on both time and instrument endpoints. Independent diagnostic workflow: https://github.com/leo14881-eng/btc-grid-state/actions/runs/36234461481. Official V5 integration guidance explicitly restricts US and mainland-China IPs. This is not an API key bug and not solvable by endless alternate-domain retries on the same US runner. The hourly GitHub scan now deliberately skips known-blocked direct Bybit requests and accepts only a complete <=45-minute official V5 snapshot from the repository's separately deployed, authorized-region collector. The full implementation is `research/hunter_bybit_regional.py`, `scripts/hunter_bybit_regional_publish.sh` and `research/reviews/2026-09-26-bybit-403-root-cause-and-regional-deployment.md`. The collector is prepared and unit-tested, but **not yet deployed**: a legitimate authorized-region machine, a least-privilege GitHub Contents write token and its actual egress country must be configured once. Until a real collector publishes a fresh valid snapshot and the GitHub health report says `bybit_complete=true`, label scope **Binance-only**, keep Bybit external blocker visible and never imply that a code test restored actual Bybit coverage. No VPN, public proxy, or regional site misrepresentation. Continue Binance research in parallel and do not spam futile 403 attempts.


## v2.17.14 — Stop ticker-only data contaminating investment prioritization

**Critical root-cause fix:** `research/hunter_forward_research.py` obtains CoinGecko and DefiLlama market rows by unique ticker, which does NOT prove token or protocol identity. Example: the Binance `AI` research row was paired with CoinGecko `artificial-inu-3` on ticker alone; do not assume this is the Binance AI asset. All such market caps, supply and protocol TVL are explicitly UNVERIFIED and no longer count as independent evidence when prioritizing dossiers; ticker-only DefiLlama TVL changes no longer trigger research-attention bonuses. Continue collecting such rows only as possible investigation leads. Only source-verified exact token contract and independently matched CoinGecko ID (or separately verified native-asset identity) can establish identity. Do not call 490 ticker-level market screens '490 fundamentally vetted assets'. Main unsolved blocker is independent, dated official forward supply/unlocks, causal token value capture and catalysts with defensible market-cap scenarios for promising candidates; software tests cannot synthesize these facts or force a trade. Report zero BUY honestly until independently validated, and separately label external Bybit dual-host 403 as an access restriction rather than a retryable code defect.


## v2.17.15 — User-approved Binance-only Hunter universe (supersedes Bybit scan requirements)

**Explicit user instruction 2026-09-26: Hunter altcoin discovery, market scanning, research candidate generation, and orderbook execution-evidence collection shall use Binance USDT spot ONLY. Do not call Bybit APIs, consume regional Bybit snapshots, deploy regional Bybit collectors, request Bybit credentials, or treat Bybit absence/403 as a Hunter bug or missing universe coverage.** This supersedes all prior Binance+Bybit union/Bybit regional collector instructions within Hunter discovery. The GitHub workflow `hunter-cex-universe.yml` is Binance-only, with `required_venues=["binance"]`, `coverage_status=BINANCE_COMPLETE` on successful complete Binance scans, and `exchange_scope=BINANCE_ONLY` in health. Count Binance coverage against the actual active Binance USDT spot listings for that scan, not against all worldwide tokens or Bybit. Retain all existing contract-identity, future supply/unlock, tokenholder value capture, credible catalyst, current Binance book/liquidity, portfolio 20,000 USDT original-cost cap, counterparty, and prospective validation gates. Do not manufacture a BUY. **This change concerns Hunter's market-data scope only**: BTC Sentinel's separate monitoring of the user's actual Bybit custody/exchange solvency risk remains unchanged and must not be disabled. Existing Bybit code may remain dormant for archival compatibility but is not a Hunter production dependency. When summarizing unresolved Hunter issues, omit Bybit 403 entirely and focus on verifiable primary-source fundamentals, economic scenarios, data limits and real forward outcomes.


## v2.17.16 — Candidate research closure, not infrastructure theater

User priority: stop optimizing already-functioning Binance collection and **finish specific candidate research**. Current primary research files: `research/hunter-reviewed-watchlist.json` (dated manually corroborated findings and unresolved event flags), `research/reviews/2026-09-26-actionable-candidate-triage.md` (DOLO/2Z/AERO/ENA/COMP fact-specific review), and hourly `research/results/hunter-candidate-dossiers.json` / `hunter-health-and-queue.json`. Retain reviewed candidates even when flow rotation changes, reserve Binance live orderbooks and source discovery for them, and show their specific next research action. DOLO official contract+CoinGecko `dolomite` identity registry now seeded; count independent identity as verified ONLY when live audit corroborates it. DOLO's official 1bn original allocation and oDOLO pairing are documented, but liquid DOLO does not earn unconditional fees and its forward sellable supply is unverified. 2Z has an independently published projected material October 2026 supply expansion; reconcile actual on-chain release around Oct 2, and do not produce new capital proposals while `PENDING_ONCHAIN_RECONCILIATION`. Theoretical constant-market-cap dilution is NOT a price prediction. AERO/ENA remain continuation research even after prior rallies. All BUY gates and the 20,000 USDT alt original-cost cap remain; no automatic orders. **Hourly reporting must distinguish newly retained reviewed candidates, new identity corroborations, actual live depth, newly closed fundamental questions, unresolved large supply risks, and capital-ready status**. Do not present software test count, market-data-screened count or generic 40 dossiers as investment results. If no new source-backed fact, do not pretend progress. Hunter uses Binance only; Bybit remains relevant solely to independent custody counterparty monitoring outside Hunter.
